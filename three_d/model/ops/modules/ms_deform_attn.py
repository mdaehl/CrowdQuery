...
if (
    ...
    or (
        float(torch.__version__.split(".")[0]) == 1
        and float(torch.__version__.split(".")[1])
    )
    < 9
):
...
class MSDeformAttn(nn.Module):
    def __init__(
        self, d_model=256, n_levels=4, n_heads=8, n_points=4, conditional=False
    ):
        """
    ...
    def forward(
        ...
        if self.conditional:
            value = value.view(
                N, Len_in, self.n_heads, (self.d_model // 2) // self.n_heads
            )
        else:
