...
class DepthAwareTransformer(nn.Module):
    def __init__(
        ...
        use_densities = density_decoder_cfg["use_densities"]
        del density_decoder_cfg["use_densities"]
        assert len(use_densities) == num_decoder_layers
        decoder_layers = [
            DepthAwareDecoderLayer(
                use_density,
                density_first,
                density_decoder_cfg,
                d_model,
        ...
        if two_stage:
            ...
            self.tgt_embed = nn.Embedding(
                self.two_stage_num_proposals * group_num, d_model
            )
    ...
    def forward(
        ...
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=srcs[0].device
        )
        ...
        if self.two_stage:
            ...
            init_box_proposal = torch.gather(
                output_proposals, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 6)
            ).sigmoid()  # sigmoid
            if self.training:
                ...
                tgt_ = self.tgt_embed.weight[
                    : self.two_stage_num_proposals, None, :
                ].repeat(1, bs, 1)  # nq, bs, d_model
        ...
        hs, inter_references, inter_references_dim = self.decoder(
            ...
            density_embed,
            query_embed
            if (not self.use_dab and not self.two_stage_dino)
            else None,  # ,INFo
            mask_flatten,
            depth_pos_embed,
            mask_depth,
...
class DepthAwareDecoderLayer(nn.Module):
    def __init__(
        ...
        if use_density:
            self.density_attn_block = DensityDecoderLayerBlock(**density_decoder_cfg)
        else:
            self.density_attn_block = None
    ...
    def forward_depth(self, tgt, bs, depth_pos_embed, mask_depth, query_pos):
        # depth cross attention
        ...
        if self.training:
            num_noise = num_queries - self.group_num * 100
            num_queries = self.group_num * 100
    ...
    def forward(
        ...
        if self.density_first:
            if self.density_attn_block is not None:
                tgt = self.density_attn_block(
                    tgt,
                    pos_embed=query_pos,
                    density_embed=density_embed,
                    density_mask=mask_depth,
                )
            tgt = self.forward_depth(
                tgt=tgt,
                bs=bs,
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 6:
                reference_points_input = (
                    ...
                    * torch.cat(
                        [src_valid_ratios, src_valid_ratios, src_valid_ratios], -1
                    )[:, None]
                )
            else:
...
def build_depthaware_transformer(cfg):
    return DepthAwareTransformer(
        density_first=cfg["density_first"],
        density_decoder_cfg=cfg["density_module"]["decoder"],
