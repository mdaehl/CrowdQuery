...
class DepthPredictor(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.depth_classifier = nn.Conv2d(
            d_model, depth_num_bins + 1, kernel_size=(1, 1)
        )
    ...
    def forward(self, feature, mask, pos):
        ...
        src_32 = self.upsample(
            F.interpolate(feature[2], size=src_16.shape[-2:], mode="bilinear")
        )
        ...
        weighted_depth = (depth_probs * self.depth_bin_values.reshape(1, -1, 1, 1)).sum(
            dim=1
        )
        # ipdb.set_trace()
