...
from density.predictor.conv import RegressionConvDensityPredictor
...
def get_gt_boxes(targets: dict, img_width: int, img_height: int) -> List[Tensor]:
    boxes_list = [target["boxes"] for target in targets]
    boxes_per_batch = [len(batch_boxes) for batch_boxes in boxes_list]
    boxes = torch.cat(boxes_list, dim=0)
    boxes[:, [0, 2]] *= img_width
    boxes[:, [1, 3]] *= img_height

    # convert from center_x, center_y, w, h -> x1, y1, x2, y2
    x_c, y_c, w, h = boxes.unbind(-1)
    boxes = [(x_c - 0.5 * w), (y_c - 0.5 * h), (x_c + 0.5 * w), (y_c + 0.5 * h)]
    boxes = torch.stack(boxes, dim=-1)
    boxes_list = list(torch.split(boxes, boxes_per_batch))
    return boxes_list


class CQ3D(nn.Module):
    def __init__(
        self,
        backbone,
        ...
        if self.two_stage_dino:
            ...
            self.depthaware_transformer.enc_out_class_embed = copy.deepcopy(
                _class_embed
            )
        ...
        if with_box_refine:
            ...
            self.dim_embed_3d = nn.ModuleList(
                [self.dim_embed_3d for _ in range(num_pred)]
            )
            self.angle_embed = nn.ModuleList(
                [self.angle_embed for _ in range(num_pred)]
            )
            self.depth_embed = nn.ModuleList(
                [self.depth_embed for _ in range(num_pred)]
            )
        ...
        density_predictor_cfg = density_config["predictor"]
        density_predictor = density_predictor_cfg["type"]
        model_mode = density_predictor_cfg["mode"]
        del density_predictor_cfg["type"]
        del density_predictor_cfg["mode"]
        if density_predictor == "classic":
            if model_mode == "regression":
                self.density_branch = RegressionConvDensityPredictor(
                    **density_predictor_cfg
                )
            else:
                raise ValueError(
                    f"{model_mode} is no valid mode for {density_predictor}."
                )
        else:
            raise ValueError(f"{density_predictor} is no valid density module.")

    def forward(self, images, calibs, targets, img_sizes, dataset, dn_args=None):
        ...
        density_logits = None
        if isinstance(self.density_branch, RegressionConvDensityPredictor):
            _, density_maps, density_embed = self.density_branch(
                srcs, pos[1], masks[1].flatten(1)
            )
        else:
            raise ValueError("Invalid density predictor.")
        ...
        for lvl in range(hs.shape[0]):
            ...
            # kitti uses different dim order
            if dataset == "kitti":
                depth_geo = (
                    size3d[:, :, 0] / box2d_height * calibs[:, 1, 1].unsqueeze(1)
                )
            else:
                depth_geo = (
                    size3d[:, :, 1] / box2d_height * calibs[:, 1, 1].unsqueeze(1)
                )
            ...
            # depth_map
            outputs_center3d = (
                ((outputs_coord[..., :2] - 0.5) * 2).unsqueeze(2).detach()
            )
        ...
        out["pred_density_maps"] = density_maps
        out["pred_density_logits"] = density_logits
...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        self.density_loss_mode = density_config["mode"]
        if self.density_loss_mode == "regression":
            self.density_loss_func = RegressionDensityLoss(
                **density_config["loss_settings"]
            )
            self.density_map_encoder = DensityMapEncoder(
                **density_config["encode_settings"]
            )
        else:
            raise ValueError(
                f"Invalid loss mode selected with {self.density_loss_mode}"
            )
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        # depth map dims (width, height)
        depth_map_dims = torch.tensor(depth_map_logits.shape[2:][::-1], device="cuda")
        # repeat as we have four points
        depth_map_dims = depth_map_dims.repeat(1, 2)
        gt_boxes2d = torch.cat([t["boxes"] for t in targets], dim=0) * depth_map_dims
    ...
    def get_density_loss(self, outputs: dict, targets: dict) -> Dict[str, Tensor]:
        # Density loss
        density_map_shape = tuple(outputs["pred_density_maps"][0].shape)
        density_map_height = density_map_shape[0]
        density_map_width = density_map_shape[1]
        bbox_list = get_gt_boxes(
            targets, img_height=density_map_height, img_width=density_map_width
        )

        gt_density_maps, gt_density_maps_norm = (
            self.density_map_encoder.transform_to_density_map(
                bbox_list, img_height=density_map_height, img_width=density_map_width
            )
        )

        ### debug section ###
        # density_map_vis = gt_density_maps[0].cpu().numpy()
        # height, width = density_map_vis.shape
        # density_map_vis = cv2.resize(density_map_vis, (int(width * 8), int(height * 8)))
        # plt.imshow(density_map_vis)
        # plt.colorbar()
        # plt.savefig("density.png")
        #####################

        if self.density_loss_mode == "regression":
            pred_density_maps = outputs["pred_density_maps"]
            density_map_loss = self.density_loss_func(
                pred_density_maps, gt_density_maps, bbox_list
            )
        else:
    ...
    def forward(self, outputs, targets, mask_dict=None, info=None):
        ...
        density_loss = self.get_density_loss(outputs, targets)
...
def build(cfg):
    # density settings
    density_config = cfg["density_module"]
    density_pred_config = density_config["predictor"]
    density_loss_config = cfg["density_loss"]

    # backbone
    backbone = build_backbone(cfg)

    model_mode = density_pred_config["mode"]
    loss_mode = density_loss_config["mode"]
    if model_mode != loss_mode:
        raise ValueError("Using different modes for the model and the loss is invalid.")

    # std is ensured via yaml
    # detr
    ...
    weight_dict["loss_density_map"] = cfg["density_map_loss_coef"]
    ...
    criterion = SetCriterion(
        ...
        losses=losses,
        density_config=density_loss_config,
    )
